...
- **Key Features**: 화면 분할 모드(SplitView), 네비게이터(Mini-map), EXIF/GPS 정보 패널, TIF Stack 지원, 이미지 일괄 변환(Batch Converter)

## 3. 빌드 지침 (Build Instructions)
- **빌드 도구**: PyInstaller (via `build.py`)
- **빌드 실행 규칙**: `build.py`를 사용하여 버전 정보를 자동 갱신하며, 최종 결과물은 ZIP으로 압축됨.
- **아이콘**: `resources/ds_viewer.ico` 사용 (인라인 SVG 아이콘은 `ui/icon_data.py` 관리)

## 4. 작업 시 주의사항 (Important Notes)
- **파일명 준수**: 진입점은 `main.py`를 사용하며, 모듈화된 구조를 유지함.
- **빌드 정보 자동화**: `build.py`가 `core/version.py` 내의 `BUILD_VERSION`, `BUILD_DATE`, `BUILD_TIME`을 관리함.
...
- **작업표시줄 그룹화**: `AppUserModelID`를 `'ds.imageviewer.v2'`로 설정하여 작업표시줄에서 독립된 아이콘으로 표시되도록 함.
